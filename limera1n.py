...
def limera1n_libusb1_async_ctrl_transfer(device, bmRequestType, bRequest, wValue, wIndex, data, timeout):
    ...
    packedStruct = struct.pack('<BBHHH', bmRequestType, bRequest, wValue, wIndex, len(data))

    dataInBytes = bytes(data, 'utf_8')
    
    request = array.array('B', struct.pack('<BBHHH', bmRequestType, bRequest, wValue, wIndex, len(data)) + dataInBytes)
