...
def Deb(msg = None):
  print(f"{os.path.basename(__file__)}, line {sys._getframe().f_back.f_lineno} - {msg if msg is not None else ''}")

# Does not find the library, why?

# lib parameter correctly identified as libusb-1.0.0.dylib

# name parameter = 'Libusb 1'

def load_library(lib, name=None, lib_cls=None):

    #Deb("Entering load_library function")
    ...
    * lib_cls    -- library class. Defaults to None (-> ctypes.CDLL).
    """
    try:
        #Deb("Entering try block")
        if lib_cls:
            #Deb("lib_cls evaluated to True")
            return lib_cls(lib)
        else:
            #Deb("lib_cls evaluated to False")
            return ctypes.CDLL(lib)
    except Exception:
        #Deb("Entering except block, ctypes.CDLL(lib) failed")
        if name:
            #Deb("name evaluated to True")
            lib_msg = '%s (%s)' % (name, lib)
        else:
            #Deb("name evaluated to False")
            lib_msg = lib
...
def load_locate_library(candidates, cygwin_lib, name,
    ...
    * LibraryMissingSymbolsException
    ...
    #Deb("Entering load_locate_library function")

    if sys.platform == 'cygwin':
        print("Platform is cygwin")
        if cygwin_lib:
            loaded_lib = load_library(cygwin_lib, name, cygwin_cls)
        else:
            raise NoLibraryCandidatesException(name)
    elif candidates:
        #Deb("Candidates does not evaluate to False")
        lib = locate_library(candidates, find_library)
        #Deb("Passed locate_library function")
        if lib:
            if sys.platform == 'win32':
                ...
                #Deb("Passed load_library function")
        else:
            _LOGGER.error('%r could not be found', (name or candidates))
            raise LibraryNotFoundException(name)
    else:
        #Deb("Raising NoLibraryCandidatesException")
        raise NoLibraryCandidatesException(name)

    if loaded_lib is None:
        #Deb("Raising LibraryNotLoadedException, loaded_lib is None")
