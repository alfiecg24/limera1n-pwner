import hashlib, os, platform, tarfile, sys
...
configs = [
    VersionConfig(
        version='12.0',
        bottle='libusb-1.0.26.monterey.bottle',
        bottle_sha256='fb0960310abaf74c8c2d4677a9531d270e03aa738f76cb367a289cda937e4ca6',
        dylib_patches=[],
        dylib_sha256='708e91ec295514c93b29fa5460f3727bd221b104bd7c036778f361e9ec299332'),
    VersionConfig(
        version='10.14',
        bottle='libusb-1.0.22.mojave.bottle',
        bottle_sha256='6accd1dfe6e66c30aac825ad674e9c7a48b752bcf84561e9e2d397ce188504ff',
        dylib_patches=[(0x8fd1, 'E985000000')],
    ...
    VersionConfig(
        ...
        dylib_patches=[(0x9917, 'E956010000')],
...
def libusb1_path_internal():
    ...
    for config in configs:
        if version.startswith(config.version):
            ...
            bottle = f.read()
            f.close()
            shaFromBottle = hashlib.sha256(bottle).hexdigest()
            expected = config.bottle_sha256
            if shaFromBottle != expected:
                sys.exit(1)
